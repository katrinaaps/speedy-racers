        ...
        <>
          <div className="absolute top-6 left-6 flex gap-3">
            ...
            <div className="bg-black/60 backdrop-blur-sm rounded-xl px-5 py-3">
              ...
              <div className="text-white text-3xl font-bold">{level}/{maxLevel}</div>
            </div>
          </div>
      ...
      {phase === "finished" && winner && (
        <div className="flex items-center justify-center h-full">
          <div className="bg-black/80 backdrop-blur-md rounded-2xl p-10 text-center pointer-events-auto">
            <div className="text-5xl font-black text-white mb-2">🏁 LEVEL {level} COMPLETE</div>
            <div className="text-3xl font-bold text-yellow-400 mb-6">
              {winner === "You" ? "YOU WIN! 🎉" : `${winner} wins!`}
            </div>
            <div className="flex gap-4">
              {winner === "You" && level < maxLevel && (
                <button onClick={onNextLevel}
                  className="bg-green-600 hover:bg-green-500 text-white font-bold text-xl px-8 py-4 rounded-xl transition-colors">
                  ▶ Next Level
                </button>
              )}
            ...
            {winner === "You" && level >= maxLevel && (
              <div className="text-2xl font-bold text-yellow-300 mt-4">🏆 ALL LEVELS COMPLETE! 🏆</div>
            )}
          </div>
        </div>
      )}
